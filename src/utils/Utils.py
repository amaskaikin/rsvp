...
def get_reservation_info(data, msg_type):
    src_ip = None
    dst_ip = None
    tos = None
    req_speed = None
    vclass = Const.CL_ADSPEC if msg_type == 'Path' else Const.CL_FLOWSPEC
    ...
    adspec_layer = get_layer(data, vclass)
    ...
    Logger.logger.info('Reservation ' + msg_type + ' request: src ip - ' + src_ip + ' dst ip - ' + dst_ip +
